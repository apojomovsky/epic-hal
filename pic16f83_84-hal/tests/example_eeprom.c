 ...
 * buffer round-trips. On this family the data pair (EEDATA/EEADR)
 * lives in Bank 0 and the control pair (EECON1/EECON2) in Bank 1. */
 ...
 * @param bank the bank to select (0..1).
...
int main(void)
{
    ...
    pic16f83_84_sim_drive_eeprom_byte(0x42U, 0xA5U);
    uint8_t r = EPIC_EEPROM_ReadByte(0x42U);
    CHECK(r == 0xA5U, "Read 0x42 returned wrong value");
    /* EEADR is Bank 0 on this family. */
    CHECK(b_read(0, PIC_REG_EEADR) == 0x42U, "EEADR != 0x42 after read");
    /* RD is set by the read and clears on the next instruction
     * cycle; the sim backend does not model the auto-clear, so the
     * test only verifies the bit was set. EECON1 is Bank 1. */
    CHECK((b_read(1, PIC_REG_EECON1) & 0x01U) == 0x01U, "RD not set after read");

    /* 2. Write: do the unlock sequence. */
    pic16f83_84_sim_reset();
    EPIC_EEPROM_WriteByte(0x10U, 0xC3U);
    /* Verify EEDATA + EEADR (Bank 0 on this family). */
    CHECK(b_read(0, PIC_REG_EEDATA) == 0xC3U, "EEDATA not 0xC3 after write");
    CHECK(b_read(0, PIC_REG_EEADR) == 0x10U, "EEADR not 0x10 after write");
    /* Verify unlock sequence: EECON2 was 0xAA last (Bank 1). */
    CHECK(b_read(1, PIC_REG_EECON2) == 0xAAU, "EECON2 != 0xAA after unlock");

    /* 3. Sim completion (EEIF is EECON1<4> on this family). */
    pic16f83_84_sim_drive_eeprom_done(0x10U, 0xC3U);
    ...
    for (uint8_t i = 0; i < 3; i++) {
        pic16f83_84_sim_drive_eeprom_done((uint8_t)(0x20U + i), data[i]);
    }
