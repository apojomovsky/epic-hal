...
def emit_build_script(manifest, module, mcu, build_dir, dfp_dir, fosc_hz=None,
    ...
    ``epic-cc --device p...`` invocation with the same source set, no DFP,
    ...
    if toolchain == "epic-cc":
        ...
        device = _device_for_epic_cc(mcu)
        ...
        # Epic-cc's CLI: --device, -I, -D are forwarded to clang; -o is hex.
        cmd = f"{epic_cc} --device {device} {inc_flags} {def_flags} -o {target} {srcs}"
        out = [
            "#!/bin/sh",
...
def _device_for_epic_cc(mcu: str) -> str:
    ...
    low = mcu.lower()
    return low if low.startswith("p") else f"p{low}"
...
def main():
    ...
    b.add_argument("--toolchain", choices=["xc8", "epic-cc"], default="xc8",
                   help="xc8 (default): emit an xc8-cc script with DFP and "
                        ...
                        "invocation with --device and EPIC_CONFIG.")
