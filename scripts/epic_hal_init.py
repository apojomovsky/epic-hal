...
def emit_makefile(manifest, family_name, part, modules, bundle_dir, name, toolchain="epic-cc") -> str:
    """The QUICKSTART.md six-line form, filled. `modules` are short names
    (epic- prefix dropped, as EPIC_HAL_MODULES expects). `toolchain`
    sets the default TOOLCHAIN variable (epic-cc or xc8)."""
    fam = manifest.families[family_name]
    mods = " ".join(modules)
    default_toolchain = toolchain if toolchain in ("epic-cc", "xc8") else "epic-cc"
    return "\n".join([
        ...
        "# Toolchain: epic-cc (default, no Microchip pack) or xc8 (alternate).",
        "# `make TOOLCHAIN=xc8` builds the same sources with XC8.",
        f"TOOLCHAIN ?= {default_toolchain}",
        "",
...
def init_project(manifest, family_name, part, modules, bundle_dir, out_dir, name, toolchain="epic-cc") -> dict:
    ...
    (out / "Makefile").write_text(
        emit_makefile(manifest, family_name, part, short, epic_hal_dir, name, toolchain=toolchain))
