/* Shared PIC14 mid-range IRQ table contract (87XA + 88X): the
 * per-source flag/enable descriptor, the PIR address selector and the
 * table itself. Each family defines the table against its own IRQn enum
 * in src/core/pic16_irq_table.c; the shared pic14_irq.c body consumes
 * it through this header. pir_reg_addr stays a macro: a function-call
