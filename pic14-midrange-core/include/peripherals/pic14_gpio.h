/* Shared PIC14 mid-range GPIO port driver (87XA parts; the 88X ANSEL
 * variant keeps its own driver: different hardware). Cube-style
 * (GPIOx, GPIO_PIN_n). Writes OR/AND the PORTx latch and never
 * read-modify the pin level first (DS39582B §4.x). PORTA is 6-bit,
 * PORTB/C/D 8-bit, PORTE 3-bit (40/44-pin only); the 628A has no
 * PORTC/D/E (PIC14MIDRANGE_HAS_PORTC/D/E). */
 ...
 *
 * PORTA and PORTB are present on every part. PORTC..PORTE depend on the
 * part (40/44-pin parts have PORTD/E, DS39582B §4.4, §4.5; small parts
 * have no PORTC/D/E).
 */
typedef enum {
