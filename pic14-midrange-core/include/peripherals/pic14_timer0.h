/* Shared PIC14 mid-range Timer0 driver (87XA + 88X), 8-bit timer/counter
 * with the shared prescaler. Sources: DS39582B §5 (87XA) and DS40001291H
 * §5 (88X); full reference: the family MANUAL.md. Writing TMR0 clears
 * the prescaler (§5.3 Note); the driver never touches PSA while WDT is
 * active (switching prescalers needs §5.3 footnote 1's sequence to avoid
 * a spurious reset). */

#ifndef PIC14_TIMER0_H
#define PIC14_TIMER0_H
 ...
 *         DS40001291H §5.3 Note). The reload-first order mirrors the 88X
 *         prescaler-switch sequence (§5.1.3.1), avoiding the spurious
 *         reset of 88X errata item 10 (DS80000302K) when the assignment
 *         changes.
 *
