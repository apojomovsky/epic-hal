 ...
 *        (SSPMSK) and adds 1110/1111 I2C slave Start/Stop variants;
 *        the 87XA uses 1001/1010 as 7/10-bit slave with Start/Stop.
 *        The SPI-slave SS-polarity names (0x4/0x5) also swap between
 *        the datasheets, so each family keeps its own spellings.
 */
typedef enum {
    ...
    SSP_MODE_I2C_SLAVE_7BIT       = 0x6U,   /**< 0110, I2C slave, 7-bit. */
    SSP_MODE_I2C_SLAVE_10BIT      = 0x7U,   /**< 0111, I2C slave, 10-bit. */
    SSP_MODE_I2C_MASTER_FOSC      = 0x8U,   /**< 1000, I2C master, Fosc/(4*(SSPADD+1)). */
    SSP_MODE_I2C_LOAD_MASK        = 0x9U,   /**< 1001, load SSPMSK (SSPADD address accesses SSPMSK). */
    SSP_MODE_I2C_MASTER_FW        = 0xBU,   /**< 1011, I2C firmware-controlled master (slave idle). */
    SSP_MODE_I2C_SLAVE_7BIT_SS    = 0xEU,   /**< 1110, I2C slave, 7-bit + Start/Stop interrupts. */
    SSP_MODE_I2C_SLAVE_10BIT_SS   = 0xFU,   /**< 1111, I2C slave, 10-bit + Start/Stop interrupts. */
#else
    SSP_MODE_SPI_SLAVE_SS_DIS     = 0x4U,   /**< 0100, SPI slave, SS disabled. */
    SSP_MODE_SPI_SLAVE_SS_EN      = 0x5U,   /**< 0101, SPI slave, SS enabled. */
    SSP_MODE_I2C_SLAVE_7BIT       = 0x6U,   /**< 0110, I2C slave, 7-bit. */
    SSP_MODE_I2C_SLAVE_10BIT      = 0x7U,   /**< 0111, I2C slave, 10-bit. */
    SSP_MODE_I2C_MASTER_FW        = 0x8U,   /**< 1000, I2C master, firmware. */
    SSP_MODE_I2C_SLAVE_7BIT_SS    = 0x9U,   /**< 1001, I2C slave, 7-bit + S/S. */
    SSP_MODE_I2C_SLAVE_10BIT_SS   = 0xAU,   /**< 1010, I2C slave, 10-bit + S/S. */
    SSP_MODE_I2C_MASTER_FOSC      = 0xBU,   /**< 1011, I2C master, hardware. */
#endif
} SSP_ModeTypeDef;

/**
 * @brief SPI clock-edge select (SSPSTAT<CKE>).
...
/**
 * @brief SPI clock polarity (SSPCON<CKP>).
 */
typedef enum {
...
/**
 * @brief SPI input-data sample phase (SSPSTAT<SMP>).
...
typedef struct {
    ...
    uint8_t                   SSPADD;          /**< I2C slave address or master baud reload. */
...
/**
 * @brief Clear the WCOL flag (must be done in software by the user).
 */
...
/**
 * @brief  Compute SSPADD for an I2C master baud rate.
 *         Fscl = Fosc / (4 * (SSPADD + 1))
 *         -> SSPADD = (Fosc / (4 * Fscl)) - 1.
 * @param fosc_hz the oscillator frequency in Hz.
 * @param fscl_hz the desired I2C clock frequency in Hz.
...
/**
 * @brief  Load the I2C address-mask register (SSPMSK). To access
 *         SSPMSK, the MSSP must first be put in the Load-Mask mode
 *         (SSPM = 1001); this function performs the mode switch, writes
 *         the mask, and restores the requested operating mode. A mask
 *         bit of 1 means the corresponding received address bit IS
 *         compared to SSPADD.
