/* Shared PIC14 mid-range host-simulation implementation of
 * EPIC_WDT_Refresh / EPIC_Sleep_Enter. Linked by the CMake host build;
 * the target twin is the _target.c. No watchdog and no halted execution
 * on the host, so both are no-ops. */
 ...
 *        callers should keep driving the family sim_step() to advance
 *        time.
 */
