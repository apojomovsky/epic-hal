/*
 * EUSART driver, async + sync master/slave (DS39605F §16.0), same shape
 * as `pic18fxx5x-hal` but with the baud control register BAUDCTL (0xFAA)
 * rather than BAUDCON. BAUDCTL has no ABDOVF bit; the ABDOVF accessors of
 * the 4550/2520 drivers are absent here.
 */

#ifndef PIC18F1320_USART_H
#define PIC18F1320_USART_H

#include "pic18f1320.h"
#include "pic18f1320_sfr.h"

/**
 * @brief USART mode (TXSTA<SYNC>, DS39605F Register 16-1).
 */
typedef enum {
...
/**
 * @brief Synchronous clock source (TXSTA<CSRC>, Register 16-1).
...
/**
 * @brief High/low baud-rate divisor (TXSTA<BRGH>, DS39605F §16.2).
 *        Combined with @ref USART_BaudGenTypeDef per Table 16-1.
 */
typedef enum {
...
/**
 * @brief Baud-generator width (BAUDCTL<BRG16>, DS39605F Register 16-3).
...
/**
 * @brief Clear an overrun: clear CREN, then re-set it (DS39605F §16.3.4).
 */
void EPIC_USART_ClearOverrun(void);

/**
 * @brief  Start auto-baud detection (sets BAUDCTL<ABDEN>). The hardware
