/*
 * Timer3 driver, 16-bit timer/counter (DS39605F §14.0), a second
 * 16-bit timer alongside Timer1. Shares Timer1's T1OSC (no T3OSCEN of
 * its own); overflow sets PIR2<TMR3IF>. T3CON<T3CCP1> selects Timer1
 * vs Timer3 as the CCP capture/compare source, left at reset (Timer1)
 * here; the CCP/ECCP driver manages it when needed. Unlike 4550/2520,
 * T3CON<6> (T3CCP2) is unimplemented on this part: no CCP2 exists.
 */

#ifndef PIC18F1320_TIMER3_H
#define PIC18F1320_TIMER3_H

#include "pic18f1320.h"
#include "pic18f1320_sfr.h"

/**
 * @brief Timer3 clock source (T3CON<TMR3CS>, DS39605F Register 14-1).
 */
typedef enum {
...
/**
 * @brief External-clock synchronisation (T3CON<T3SYNC>, DS39605F §14.0).
...
/**
 * @brief Prescaler ratio (T3CON<T3CKPS1:T3CKPS0>, DS39605F Register 14-1).
 */
typedef enum {
 ...
 *  T1OSC (enable it via the Timer1 driver if needed). No CCP2 select:
 *  this part has only a single ECCP1, so there is no T3CCP2 bit. */
typedef struct {
