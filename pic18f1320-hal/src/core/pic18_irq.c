 ...
 * compile-time-constant `PIC_REG_*` token, never a runtime address: a
 * runtime SFR address compiles to the program-memory table mechanism
 * on PIC18 (see `pic18fxx5x-hal/README.md`). GIEH/GIEL act as one
 * on/off switch; enabling also sets IPEN for the two-vector scheme.
 */
 ...
 *         (INTCON<GIEH/GIEL>, DS39605F §9.0). In priority mode both GIEH
...
{
    if (prev_state) {
        /* Activate the two-vector priority scheme (DS39605F §9.0, RCON<7>)
 ...
 *         in INTCON / INTCON3 / PIE1 / PIE2 per the source.
