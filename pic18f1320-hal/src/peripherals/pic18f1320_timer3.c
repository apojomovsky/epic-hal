/*
 * Timer3 driver, implementation (DS39605F §14.0, Register 14-1).
 */

#include "peripherals/pic18f1320_timer3.h"
#include "core/pic18_irq.h"

/* T3CON prescaler ratios, DS39605F Register 14-1:
 ...
 *         latches TMR3H into a shadow, so reading low then high yields a
 *         consistent 16-bit value.
 * @return The current TMR3H:TMR3L value.
 */
uint16_t EPIC_TIMER3_ReadCounter(void)
{
    /* With RD16 set, reading TMR3L latches TMR3H into a shadow (DS39605F
...
/**
 * @brief  Write the 16-bit Timer3 counter. With RD16 set, writing TMR3L
 *         latches TMR3H, so the high byte is written first via the shadow
...
{
    /* With RD16 set, writing TMR3L latches TMR3H (DS39605F §14.0); write
...
/**
 * @brief  Map a Timer3 prescaler enum to its ratio (DS39605F Register
...
EPIC_StatusTypeDef EPIC_TIMER3_Init(const TIMER3_HandleTypeDef *h)
{
    ...
    /* Enable 16-bit read/write mode (RD16). Leave T3CCP1 at reset
     * (0 = Timer1 is the CCP time base); the CCP/ECCP driver manages it.
     * T3CON<6> (T3CCP2) is unimplemented on this part and left clear. */
...
EPIC_StatusTypeDef EPIC_TIMER3_Start(const TIMER3_HandleTypeDef *h)
{
    ...
    /* Program T3CON in one write. RD16 stays set (from Init). T3CCP1
     * (bit 3) is left at 0 (Timer1 as CCP time base); bit 6 is
     * unimplemented on this part (no CCP2).
