# `epic-math` API reference

The authoritative declarations live in [`include/epic_math.h`](../include/epic_math.h);
...
```c
typedef struct { uint16_t quotient, remainder; } epic_math_udiv16_t;  /* unsigned 16/16 */
typedef struct { int16_t  quotient, remainder; } epic_math_sdiv16_t;  /* signed 16/16   */
```

`EPIC_MATH_OPTIMIZE_FOR_SIZE` (default `1`): the PIC16 8x8 multiply code-size
...
### `int16_t epic_math_negate_s16(int16_t v)` / `int32_t epic_math_negate_s32(int32_t v)`
...
### `uint8_t epic_math_bcd8_to_bin(uint8_t bcd2)` / `uint8_t epic_math_bin_to_bcd8(uint8_t value)`
2-digit BCD <-> 0..99.
...
### `uint8_t epic_math_bcd_add8(uint8_t a, uint8_t b, bool *carry_out)` / `uint8_t epic_math_bcd_sub8(uint8_t a, uint8_t b, bool *borrow_out)`
...
### `int16_t epic_math_diff3(int16_t x_prev2, int16_t x_prev1, int16_t x_now, int16_t inv_2h_q8)`
...
### `int32_t epic_math_integrate_simpson38(int16_t f0, int16_t f1, int16_t f2, int16_t f3, int32_t three_h_over_8_q16)`
...
| Function | Purpose |
|---|---|
| `epic_math_mul_u8/u16/s16` | unsigned/signed multiply |
| `epic_math_divmod_u16/s16/u32_16` | divide with remainder + `ok` flag |
| `epic_math_add_u16/sub_u16` | 16-bit add/sub with carry/borrow out |
| `epic_math_negate_s16/s32` | two's-complement negate |
| `epic_math_bcd8_to_bin/bin_to_bcd8` | 2-digit BCD <-> 0..99 |
| `epic_math_bcd16_to_bin/bin_to_bcd16` | 5-digit BCD <-> 0..65535 |
| `epic_math_bcd_add8/sub8` | packed-BCD add/sub with carry/borrow |
| `epic_math_sqrt_u16` | floor(sqrt), Newton on div |
| `epic_math_diff3` | 3-point central derivative, Q8.8 |
| `epic_math_integrate_simpson38` | Simpson 3/8 integration, Q16.16 |
| `epic_math_rand_next` | 16-bit LFSR PRNG (explicit state) |
| `epic_math_rand_gauss` | CLT Gaussian sample |
