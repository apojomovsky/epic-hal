...
static void test_bcd8_roundtrip(void)
{
    for (uint32_t v = 0; v <= 99u; v++) {
        ...
        CHECK(epic_math_bcd8_to_bin(ref_bcd8((uint8_t)v)) == v, "bcd8_to_bin value");
    }
}

/** @brief Documented invalid-nibble behavior of epic_math_bcd8_to_bin. */
static void test_bcd8_invalid_nibble(void)
{
    /* Documented: each nibble processed arithmetically. 0x0A -> 0*10+10 = 10. */
    CHECK(epic_math_bcd8_to_bin(0x0A) == 10u, "bcd8_to_bin invalid low nibble");
    CHECK(epic_math_bcd8_to_bin(0xA0) == 100u, "bcd8_to_bin invalid high nibble");
    CHECK(epic_math_bcd8_to_bin(0xAA) == 110u, "bcd8_to_bin both nibbles invalid");
}

/** @brief Exhaustive 0..65535 roundtrip of epic_math_bin_to_bcd16/epic_math_bcd16_to_bin. */
...
/** @brief Documented invalid-nibble and oversize-truncation behavior of epic_math_bcd16_to_bin. */
static void test_bcd16_invalid_nibble(void)
{
    /* 0x0000A -> 10; 0xABCDE has every nibble >9 -> 10+11*10+12*100+13*1000+14*10000. */
    CHECK(epic_math_bcd16_to_bin(0x0000Au) == 10u, "bcd16 invalid nibble");
    uint32_t bcd = 0xABCDEu;
    uint32_t exp = 0xEu + 0xDu*10u + 0xCu*100u + 0xBu*1000u + 0xAu*10000u;
    CHECK(epic_math_bcd16_to_bin(bcd) == (uint16_t)exp, "bcd16 all-invalid nibbles");
    ...
    CHECK(epic_math_bin_to_bcd16(65535u) == 0x65535u, "bin_to_bcd16 max");
    CHECK(epic_math_bin_to_bcd16(0u) == 0u, "bin_to_bcd16 zero");
    CHECK(epic_math_bcd16_to_bin(0x99999u) == (uint16_t)99999u, "bcd16 oversize truncates");
}

/** @brief Exhaustive 0..99 x 0..99 add of epic_math_bcd_add8 incl. carry and NULL pointer. */
static void test_bcd_add8(void)
{
    ...
    (void)epic_math_bcd_add8(0x99u, 0x01u, NULL);
}

/** @brief Exhaustive 0..99 x 0..99 subtract of epic_math_bcd_sub8 incl. borrow and NULL pointer. */
static void test_bcd_sub8(void)
{
    ...
    (void)epic_math_bcd_sub8(0x00u, 0x01u, NULL);
}
