...
/**
 * @brief  No-op TX callback placeholder; non-null so EPIC_USART_Init
 *         arms TXEN (both families gate TXEN on the callback being
 *         present). Never actually invoked: transmission here is polled
 *         and GIE stays off.
 *
...
/** @brief Transmit one byte, polling the shift register until it drains. */
...
{
    while (*s) putc_((uint8_t)*s++);
}
/** @brief Transmit a 16-bit value as 4 hex digits. */
static void putx16(uint16_t v)
{  /* 4-digit hex */
...
/** @brief Transmit a 32-bit value as 8 hex digits. */
static void putx32(uint32_t v)
{
    putx16((uint16_t)(v >> 16)); putx16((uint16_t)v);
}
/** @brief Transmit an unsigned value in decimal. */
static void putd(uint32_t v)
{  /* unsigned decimal */
    char buf[11]; int n = 0;
    if (v == 0u)
    {
...
/** @brief Log one FAIL record (label, got, exp) over USART and bump g_fail.
 *  A FUNCTION, not an inlined macro body: CHECK_EQ appears at ~150 call
 *  sites, so inlining the print would balloon the code size past the
 *  8 K-word PIC16F87XA flash. */
...
static void run_mul_u8(void)
{
    for (int i=0;i<(int)GV_MUL_U8_N;i++)   CHECK_EQ(epic_math_mul_u8(gv_mul_u8[i].a,gv_mul_u8[i].b), gv_mul_u8[i].e, "mul_u8");
}
/** @brief Replay the mul_u16 golden vectors against the on-target asm. */
static void run_mul_u16(void)
{
    for (int i=0;i<(int)GV_MUL_U16_N;i++)  CHECK_EQ(epic_math_mul_u16(gv_mul_u16[i].a,gv_mul_u16[i].b), gv_mul_u16[i].e, "mul_u16");
}
/** @brief Replay the div_u16 golden vectors against the on-target asm. */
static void run_div_u16(void)
{
    for (int i=0;i<(int)GV_DIV_U16_N;i++)
    {
        epic_math_udiv16_t r=epic_math_divmod_u16(gv_div_u16[i].n,gv_div_u16[i].d,0);
...
static void run_div_u32(void)
{
    for (int i=0;i<(int)GV_DIV_U32_16_N;i++)
    {
        epic_math_udiv16_t r=epic_math_divmod_u32_16(gv_div_u32_16[i].n,gv_div_u32_16[i].d,0);
...
{
    for (int i=0;i<(int)GV_ADD_U16_N;i++)
    {
        bool c; uint16_t r=epic_math_add_u16(gv_add_u16[i].a,gv_add_u16[i].b,&c);
...
static void run_sub(void)
{
    for (int i=0;i<(int)GV_SUB_U16_N;i++)
    {
        bool b; uint16_t r=epic_math_sub_u16(gv_sub_u16[i].a,gv_sub_u16[i].b,&b);
...
static void run_neg_s16(void)
{
    for (int i=0;i<(int)GV_NEG_S16_N;i++)  CHECK_EQ((uint16_t)epic_math_negate_s16(gv_neg_s16[i].v),(uint16_t)gv_neg_s16[i].e,"neg_s16");
}
/** @brief Replay the neg_s32 golden vectors against the on-target asm. */
static void run_neg_s32(void)
{
    for (int i=0;i<(int)GV_NEG_S32_N;i++)  CHECK_EQ((uint32_t)epic_math_negate_s32(gv_neg_s32[i].v),(uint32_t)gv_neg_s32[i].e,"neg_s32");
}
/** @brief Replay the bcd_add8 golden vectors against the on-target asm. */
static void run_bcd_add8(void)
{
    for (int i=0;i<(int)GV_BCD_ADD8_N;i++)
    {
        bool c; uint8_t r=epic_math_bcd_add8(gv_bcd_add8[i].a,gv_bcd_add8[i].b,&c);
...
static void run_bcd_sub8(void)
{
    for (int i=0;i<(int)GV_BCD_SUB8_N;i++)
    {
        bool b; uint8_t r=epic_math_bcd_sub8(gv_bcd_sub8[i].a,gv_bcd_sub8[i].b,&b);
...
/** @brief Replay all golden vectors on silicon, stream a PASS/FAIL summary, then idle. */
int main(void)
{
