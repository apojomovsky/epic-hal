/*
 * Host tests asserting epic_math_rand_next's documented LFSR properties
...
#define EPIC_MATH_RAND_SEED 0xACE1u
...
static void test_rand_period_and_zero(void)
{
    ...
    uint16_t state = EPIC_MATH_RAND_SEED;
    uint16_t first = epic_math_rand_next(&state);
    (void)first;
    uint32_t count = 1u;
    int saw_zero = 0;
...
static void test_rand_zero_state_escape(void)
{
    ...
    uint16_t state = 0u;
    uint16_t v1 = epic_math_rand_next(&state);
    ...
    uint16_t s2 = EPIC_MATH_RAND_SEED;
    (void)epic_math_rand_next(&s2);
    CHECK(state == s2, "zero-state escape joins the seed's sequence");
    uint16_t v2 = epic_math_rand_next(&state);
...
/** @brief Coarse bell-shape sanity check on epic_math_rand_gauss (not a chi-square test). */
