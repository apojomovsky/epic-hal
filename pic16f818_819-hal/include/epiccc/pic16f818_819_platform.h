 ...
 * target/pic16f818_819_platform.h and host/pic16f818_819_platform.h);
 * the build's include path picks which resolves, so
 * pic16f818_819_hal.h includes "pic16f818_819_platform.h"
 * unconditionally with no #ifdef. Banking is inserted by the
 * compiler's banking pass, so the XC8-specific inline-asm pie/bank
 * fixups are plain C here; placement pins are dropped (EPIC_PLACE
 * expands to nothing) because the overlay places globals itself. */
 ...
 * maps to its address here. The peripherals pass OPTION_REG to the
 * Bank-1 macros, whose epic-cc forms index PIC_REG_##sfr_name; the
 * family SFR map owns PIC_REG_OPTION_REG (pic16f818_819_sfr.h). */
#ifndef OPTION_REG
 ...
 * EEPROM data pair and Bank 3 the EEPROM control pair. */
...
/* PIE1/PIE2 enable-bit reads for the dispatch tiers: TMR1IE, TMR2IE,
 * CCP1IE, SSPIE and ADIE all sit in PIE1, EEIE in PIE2. No TXIE and no
 * CCP2IE reader: this die has neither USART nor CCP2. */
