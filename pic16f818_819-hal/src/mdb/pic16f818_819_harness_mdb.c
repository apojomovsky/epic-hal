 ...
 * no USART, so the PASS/FAIL marker goes out on RA0 (MODE=gpio in
 * scripts/sim-mdb-run.sh): PASS drives RA0 high, FAIL leaves it low;
 * epic_harness_log discards every other line. running() is bounded by
 * `cycles`. */
...
/**
 * @brief Harness start-up (sim target): stores the cycle bound, selects
 *        all-digital analog configuration so the RA0 marker pin has a
 *        readable digital input path, arms RA0 as the marker output
 *        (driven low = not-yet-passed), and clears any stale EEPROM
 *        completion flag from a previous run.
 * @param cycles the number of iterations the run is bounded by.
 */
void epic_harness_init(uint32_t cycles)
{
    g_cycles = cycles;
    /* ADCON1 PCFG 011x keeps every AN pin digital (DS39598F Register
     * 11-2), which the marker needs: at reset PCFG is 0000, an
     * analog-selected pin reads 0 through its digital input path
     * (Section 11.3), and the mdb gate reads the verdict off PORTA. */
...
/**
 * @brief Log line: the magic-string dispatch is trimmed for flash (see
 *        the body note). Drives RA0 high on the PASS marker, low on
 *        the FAIL marker; every other line is a no-op.
...
{
    /* Magic-string dispatch, trimmed for flash: the 193X harness
     * compares all 26 bytes of each marker, which alone costs ~200
     * words. The only callers are epic_harness_report's two fixed
     * literals, so the distinctive bytes pin the verdict:
     * "...RESULT: PASS\n\0" vs "...RESULT: FAIL\n\0" differ at
 ...
 */
void pic16f818_819_harness_halt(void)
{
    for (;;)
    {
        /* nothing */
