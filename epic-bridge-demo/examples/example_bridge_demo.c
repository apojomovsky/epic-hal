/*
 * epic-bridge-demo target build: real PIC18F4550 hardware. Spawns the
...
#define TASK_PERIOD_BRIDGE    1U    /* every tick: poll the slave */
...
int main(void)
{
    ...
    epic_taskmgr_init();
    epic_taskmgr_spawn(bridge_demo_task_bridge,    NULL, TASK_PERIOD_BRIDGE,    0U);
    epic_taskmgr_spawn(bridge_demo_task_adc,       NULL, TASK_PERIOD_ADC,       1U);
    epic_taskmgr_spawn(bridge_demo_task_heartbeat, NULL, TASK_PERIOD_HEARTBEAT, 2U);
    ...
    EPIC_IRQ_Restore(1); /* arm the Timer0 + USART + Timer2 interrupts */
