 ...
 *        dispatch entry in pic18_irq_dispatch.c. An enabled source with
 *        no dispatch entry never clears and re-vectors forever, so the
 *        rest join here as each ticket lands its dispatch entry.
 */
typedef enum {
 ...
 *        (INTCON<GIEH/GIEL>, DS39631E §9.0). In priority mode both
 ...
 *        shared `EPIC_IRQ_*` contract (the PIC16 implementation is a no-op).
 * @param irq the source to configure; prio HIGH or LOW vector.
 */
