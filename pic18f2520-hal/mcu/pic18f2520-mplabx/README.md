...
family's `USBDIV` / `CPUDIV` / `PLLDIV` / `VREGEN` fields. The manifest
`example.PIC18F2520` table carries the conservative default set used by
all 2520 builds: `FOSC = HS`, `WDT = ON` (refresh via
`EPIC_WDT_Refresh()`), `PWRT = ON`, `MCLRE = ON`, `LVP = OFF`, `XINST =
OFF`, `DEBUG = OFF`, `BOREN = ON`, `STVREN = ON`, `PBADEN = OFF`, with
code / write / read protection all `OFF`. The `CCP2MX` bit selects whether
CCP2 multiplexes onto RB3 or RC1; it is `off`-agnostic for the foundation
(no CCP driver yet, phase 3).
...
and the CI wrapper reads `print PORTA` in `MODE=gpio`. This is the
PIC16F193X pattern (see `pic16f193x-hal/mcu/pic16f193x-mplabx/README.md`);
once the EUSART driver lands in phase 3 the gates can switch to `MODE=uart`.
