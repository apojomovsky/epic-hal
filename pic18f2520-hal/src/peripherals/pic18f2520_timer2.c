/*
 * Timer2 driver, implementation (DS39631E §13.0, Register 13-1).
 */

#include "peripherals/pic18f2520_timer2.h"
#include "core/pic18_irq.h"

/* T2CON prescaler, DS39631E Register 13-1:
 *   00 -> 1:1, 01 -> 1:4, 1x -> 1:16 */
static const uint8_t pre_ratio[4] = { 1, 4, 16, 16 };

/* T2CON postscaler, DS39631E Register 13-1: 1:(N+1). */
...
/**
 * @brief  Map a Timer2 prescaler enum to its ratio (DS39631E Register
 *         13-1: 00 -> 1:1, 01 -> 1:4, 1x -> 1:16).
 * @param p Prescaler selection (T2CKPS<1:0> value).
 * @return The divide ratio (1, 4 or 16).
 */
...
/**
 * @brief  Map a Timer2 postscaler enum to its ratio (DS39631E Register
 *         13-1: 1:(N+1)).
 * @param p Postscaler selection (T2OUTPS<3:0> value).
 * @return The divide ratio (1..16).
 */
...
EPIC_StatusTypeDef EPIC_TIMER2_Start(const TIMER2_HandleTypeDef *h)
{
    ...
    /* Period register first (DS39631E §13.0 recommends setting PR2 before
