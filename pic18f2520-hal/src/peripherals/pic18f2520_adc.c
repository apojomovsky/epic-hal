/*
 * A/D converter driver, implementation (DS39631E §19.0). No bank
 * switching (all ADC registers are in the Access Bank), but three control
 * registers (ADCON0/1/2) versus PIC16's two. Conversion completion (GO/DONE clear + ADIF) is proven via mdb;
 * the host sim verifies register programming only.
 */

#include "peripherals/pic18f2520_adc.h"
...
EPIC_StatusTypeDef EPIC_ADC_Init(const ADC_HandleTypeDef *h)
{
    ...
    /* ADCON2 (Register 19-3): ADFM | ACQT2:ACQT0 | ADCS2:ADCS0. */
    uint8_t adcon2 = 0U;
...
uint16_t EPIC_ADC_Read(void)
{
     ...
     * 0..1023 result (DS39631E Register 19-3 + Figure 19-4). */
