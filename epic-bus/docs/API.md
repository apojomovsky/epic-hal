# `epic-bus` API reference

Authoritative declarations: [`include/epic_bus.h`](../include/epic_bus.h).
...
epic_bus_i2c_init(FOSC_HZ, 100000);
uint8_t who[3];
if (epic_bus_i2c_mem_read(0x50, 0x10, who, 3) == 3) { /* got them */ }

/* SPI sensor: write 2 bytes to reg 0x20 */
epic_bus_spi_init(FOSC_HZ, 0, GPIOB, 0);   /* CS on PB0 */
uint8_t cfg[2] = { 0x01, 0x02 };
epic_bus_spi_mem_write(0x20, cfg, 2);
```
...
| Function | Purpose |
|---|---|
| `epic_bus_i2c_init` | I2C master init + default ops |
| `epic_bus_i2c_mem_read` | I2C read N bytes from a register |
| `epic_bus_i2c_mem_write` | I2C write N bytes to a register |
| `epic_bus_spi_init` | SPI master init + GPIO CS + default ops |
| `epic_bus_spi_mem_read` | SPI read N bytes from a register |
| `epic_bus_spi_mem_write` | SPI write N bytes to a register |
| `epic_bus_set_i2c_ops` / `set_spi_ops` | inject alternate/mock ops |
