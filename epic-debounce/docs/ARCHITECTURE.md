...
(`EPIC_GPIO_ReadPin`), an I2C-expander bit (`epic-bus`), or a mock pin in a
test. This is the design choice that makes the module "reusable on any gpio."

## Depends on `epic-tick` directly (not an injected clock)

The algorithm needs elapsed-time. It calls `epic_tick_get()` /
`epic_tick_elapsed_since()` directly, not a second injected callback, so the
...
timing). Only `src/debounce.c` includes `epic_tick.h`; the public header
...
## Does not use `epic-fsm` internally
...
clarity gain. `epic-fsm` composability holds at the *call site*:
...
`epic_tick_get()`), so a button already held down at boot does NOT spuriously
...
small `epic-fsm` machine one level up, not built into this module.
...
(pointers are 1-2 bytes). Comparable to `epic-fsm`'s `fsm_t`.
