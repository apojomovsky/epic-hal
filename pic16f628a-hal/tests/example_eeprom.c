...
int main(void)
{
    ...
    pic16f628a_sim_drive_eeprom_byte(0x42U, 0xA5U);
    uint8_t r = EPIC_EEPROM_ReadByte(0x42U);
    CHECK(r == 0xA5U, "Read 0x42 returned wrong value");
    CHECK(b_read(1, PIC_REG_EEADR) == 0x42U, "EEADR != 0x42 after read");
    ...
    /* Verify EEDATA + EEADR. */
    CHECK(b_read(1, PIC_REG_EEDATA) == 0xC3U, "EEDATA not 0xC3 after write");
    CHECK(b_read(1, PIC_REG_EEADR) == 0x10U, "EEADR not 0x10 after write");
    /* Verify unlock sequence: EECON2 was 0xAA last. */
    CHECK(b_read(1, PIC_REG_EECON2) == 0xAAU, "EECON2 != 0xAA after unlock");

    /* 3. Sim completion. */
    pic16f628a_sim_drive_eeprom_done(0x10U, 0xC3U);
    ...
    for (uint8_t i = 0; i < 3; i++) {
        pic16f628a_sim_drive_eeprom_done((uint8_t)(0x20U + i), data[i]);
    }
