/* Timer2 driver implementation (DS40001291H §7.0). */

#include "peripherals/pic16f88x_timer2.h"
#include "core/pic16_irq.h"

/* T2CON prescaler, DS40001291H Register 7-1:
 *   00 → 1:1, 01 → 1:4, 1x → 1:16 */
static const uint8_t pre_ratio[4] = { 1, 4, 16, 16 };

/* T2CON postscaler, DS40001291H Register 7-1: 1:(N+1). */
 ...
 * avoids). Pinned to bank 2 (0x141) when the part has Bank 2 GPR
...
uint8_t EPIC_TIMER2_ReadPeriod(void)
{
     ...
     * v4.00 (see target/pic16f88x_platform.h). */
    uint8_t pr2 = 0u;
    EPIC_BANK1_READ8(PR2, pr2);
    return pr2;
#else
    /* PR2 lives in Bank 1 (DS40001291H Register 7-2, address 0x92). */
