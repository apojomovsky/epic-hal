/* Fan-out from the single PIC16 interrupt vector (0x0004, DS40001291H
 ...
 * unconditionally invoking every handler. Strong extern prototypes,
...
/** @brief PORTB RB<7:0> change ISR (weak, overridable). */
 ...
 * handlers whose flag is set. Runs in the ISR.
 *
 * Pinned to flash page 1 (0x900) on parts with >= 4K flash: XC8 emits
 * no PCLATH setup for the handler calls (it assumes the interrupt
 * call-graph is linked into one flash page), so the dispatch and
 * handlers must share a page. The PIC16F882 (2K flash, one page only)
 * needs no pin: everything lives in page 0.
 */
#if defined(__XC8) && PIC16F88X_FAMILY_FLASH_KW >= 4
