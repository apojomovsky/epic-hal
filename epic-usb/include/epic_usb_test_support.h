/**
 * @file    epic_usb_test_support.h
 * @brief   Host-stub-only driver hooks for exercising epic_usb.h's public
 *          API contract without real USB hardware.
 *
 * @details
 *   Implemented only by epic_usb_host_stub.c, not the real-target
 *   epic_usb.c: there is no faithful host simulation of the real USB SIE,
 ...
 *  while connected) since the last epic_usb_test_reset_sent(). */
size_t epic_usb_test_sent_len(void);

/** Pointer to the stub's transmitted-byte log (epic_usb_test_sent_len()
 *  bytes valid). */
