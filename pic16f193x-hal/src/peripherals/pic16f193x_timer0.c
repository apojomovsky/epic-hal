 ...
 *   handle is copied into owned static storage in HAL_TIMER0_Init (the
 ...
 *  slot is sufficient; HAL_TIMER0_Init COPIES the caller's handle here. */
...
HAL_StatusTypeDef HAL_TIMER0_Init(const TIMER0_HandleTypeDef *h)
{
    ...
    HAL_IRQ_ClearFlag(PIC16F193X_IRQ_TMR0);
    if (h->OverflowCallback) {
        HAL_IRQ_Enable(PIC16F193X_IRQ_TMR0);
    } else {
        HAL_IRQ_DisableSrc(PIC16F193X_IRQ_TMR0);
    }
...
{
    if (!HAL_IRQ_GetFlag(PIC16F193X_IRQ_TMR0)) return;
    HAL_IRQ_ClearFlag(PIC16F193X_IRQ_TMR0);
