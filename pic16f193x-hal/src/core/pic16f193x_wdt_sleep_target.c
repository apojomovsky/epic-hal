/**
 * @file    pic16f193x_wdt_sleep_target.c
 ...
 *   pic16f193x_wdt_sleep_sim.c; the build picks one, so neither this file
 *   nor the examples need `#ifdef`. On a real PIC these are the native
 *   `clrwdt` / `sleep` instructions (both present on the Enhanced
 *   Mid-range core, DS41364B §24.0 / §24.2). The shared BOR/POR status
 *   helpers live in pic16f193x_wdt_sleep.c.
 */
