/**
 * Real-target interrupt-vector entry (XC8 target build only): the single
 * PIC16F193X vector at 0x0004 (DS41364B §4.0) calls the shared
 * epic_dispatch_all_irqs fan-out; the host harness registers the same
 * dispatcher as its sim IRQ callback instead. The Enhanced Mid-range
 * core saves W/STATUS/BSR/FSR0/FSR1/PCLATH to shadow registers in
 * hardware on interrupt entry and restores them on RETFIE (DS41364B
 * §4.1), so no manual push/pop is needed here (unlike classic PIC16F87XA).
 *
 * Although the auto-context-save shape avoids the manual push/pop that
 * forced the classic family's pushpop scratch byte, the platform's
 * PIE1/2/3 read-modify-write macros still need an `__at()`-pinned
 * common-RAM scratch byte to bridge the bank-select gap: the plain-C
 * RMW compiles to FSR1:INDF1 indirect addressing with FSR1H=0, the
 * wrong bank for PIE registers (all in bank 1). Pin
 * epic_irq_pie_scratch to bank-independent common RAM at 0x70
