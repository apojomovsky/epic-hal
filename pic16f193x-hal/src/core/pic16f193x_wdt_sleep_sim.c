/**
 * @file    pic16f193x_wdt_sleep_sim.c
 ...
 *   is pic16f193x_wdt_sleep_target.c; the build picks one, so neither this
 *   file nor the examples need `#ifdef`. On the host there is no watchdog
 *   and execution does not stop, so both are no-ops. The shared BOR/POR
 *   status helpers live in pic16f193x_wdt_sleep.c.
 */
