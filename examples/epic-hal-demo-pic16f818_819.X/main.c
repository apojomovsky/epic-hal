/*
 * Epic HAL reference project, PIC16F819: Timer0 overflow toggles RB0
 * via the HAL's weak-ISR dispatch. 20 MHz HS crystal. See MPLABX.md to
 * add Epic HAL to an existing project.
 */
#include <xc.h>
 ...
 * an ISR-level driver call permanently reserves its compiled-stack path
 * (the 83_84 precedent), so the reference project keeps the ISR call
 * graph minimal on a 128/256 B RAM family.
 */
