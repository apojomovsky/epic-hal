...
`epic-serial` layers ring buffers and demand-driven interrupt TX/RX on the
...
"set the callback, don't redefine the handler" pattern `epic-taskmgr` uses for
its Timer0 tick.
...
the callbacks would read stale memory. (Same lifetime rule as `epic-tick`'s
...
`epic_dispatch_all_irqs` (the host analogue of the TX ISR firing) with TXIF
