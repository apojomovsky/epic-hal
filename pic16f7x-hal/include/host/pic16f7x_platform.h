 ...
 * target/pic16f7x_platform.h); the include path picks which resolves,
 * so pic16f7x.h includes this name unconditionally with no #ifdef.
 * SFR access indexes the 512-byte memory-backed pic16f7x_sim_sfr[]
 * (src/sim/pic16f7x_sim.c), so tests can poke registers directly. */

#ifndef PIC16F7X_PLATFORM_H
#define PIC16F7X_PLATFORM_H

#include <stdint.h>

/* 512-byte memory-backed register file (DS30325 Figure 2-3/2-4 layout),
 * defined in src/sim/pic16f7x_sim.c. */
extern uint8_t pic16f7x_sim_sfr[0x200];
...
#define EPIC_SFR_PTR(addr)       (&pic16f7x_sim_sfr[(uint16_t)(addr)])
#define epic_sfr_read8(addr)     (pic16f7x_sim_sfr[(uint16_t)(addr)])
#define epic_sfr_write8(addr, v) \
    do { pic16f7x_sim_sfr[(uint16_t)(addr)] = (uint8_t)(v); } while (0)

/* Address of a register as a uint8_t lvalue (read/write/RMW). */
#define EPIC_REG8(addr)          (pic16f7x_sim_sfr[(uint16_t)(addr)])

/* PIE1 (0x8C) / PIE2 (0x8D) enable/disable, direct read-modify-write:
 * the simulated register file is a plain array, so none of
 * target/pic16f7x_platform.h's inline-asm banking path applies here. */
#define EPIC_PIE_ENABLE_BIT(is_pir2, mask) \
    do { \
        if (is_pir2) { pic16f7x_sim_sfr[0x8DU] |= (uint8_t)(mask); } \
        else         { pic16f7x_sim_sfr[0x8CU] |= (uint8_t)(mask); } \
    } while (0)
...
#define EPIC_PIE1_READ_TMR1IE(out_var) ((out_var) = pic16f7x_sim_sfr[0x8CU])

/* Host twin of the target header's EPIC_PIE1_READ_TXIE. */
#define EPIC_PIE1_READ_TXIE(out_var)   ((out_var) = pic16f7x_sim_sfr[0x8CU])

/* Host twin of the target header's EPIC_PIE2_READ_EEIE (PIE2 at
 * 0x8D). */
#define EPIC_PIE2_READ_EEIE(out_var)   ((out_var) = pic16f7x_sim_sfr[0x8DU])

#endif /* PIC16F7X_PLATFORM_H */
