 ...
 * host/pic16f7x_platform.h); the include path picks which resolves,
...
/* Same fix, Banks 2/3 (pic16f7x_eeprom.c's EEDATA/EEADR/EECON1/
...
#else
/* PIE1-only part (16F72): no PIR2/PIE2, so the PIE2 branch would
 * reference an assembler symbol the DFP header does not define. */
#define EPIC_PIE_ENABLE_BIT(is_pir2, mask)                              \
    do {                                                                \
        (void)(is_pir2);                                                \
        epic_irq_pie_scratch = (uint8_t)(mask);                        \
...
#else
/* PIE1-only part (16F72): no PIR2/PIE2. */
#define EPIC_PIE_DISABLE_BIT(is_pir2, mask)                             \
    do {                                                                \
        (void)(is_pir2);                                                \
        epic_irq_pie_scratch = (uint8_t)~(mask);                       \
