/* Host simulation backend for the PIC16F7X HAL: provides the 512-byte
 * memory-backed register file the host SFR macros dereference and the
 * peripheral models (Timer0/1/2, ADC, USART, MSSP; no EEPROM on the 7x). The sim
 * never bit-bangs external pins; the test rig drives and observes them
 * through pic16f7x_sim.h's helpers. */

#include "pic16f7x_sim.h"
...
uint8_t pic16f7x_sim_sfr[0x200];
...
/* Optional ISR hook. */
static pic16f7x_sim_irq_cb_t sim_irq_cb = 0;
...
static uint8_t port_latch(char port)
{
    switch (port)
    {
        ...
        case 'D': case 'd': return pic16f7x_sim_sfr[PIC_REG_PORTD];
        case 'E': case 'e': return pic16f7x_sim_sfr[PIC_REG_PORTE];
...
static uint8_t tris_reg(char port)
{
    switch (port)
    {
        ...
        case 'D': case 'd': return pic16f7x_sim_sfr[PIC_REG_TRISD];
        case 'E': case 'e': return pic16f7x_sim_sfr[PIC_REG_TRISE];
        default:             return 0xFFU;
    }
}
 ...
 */
void pic16f7x_sim_reset(void)
{
    memset(pic16f7x_sim_sfr, 0, sizeof pic16f7x_sim_sfr);

    /* Power-on reset values from DS30325 Table 14-6. */
    pic16f7x_sim_sfr[PIC_REG_STATUS]   = PIC_STATUS_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_PCON]     = PIC_PCON_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_INTCON]   = PIC_INTCON_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_PIR1]     = PIC_PIR1_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_PIR2]     = PIC_PIR2_POR_VALUE;
    /* PIR1 <TXIF> resets to 1 (TXREG empty after POR, §10.2.1).
     * Bit 4 of PIR1 (at 0x0C). */
    pic16f7x_sim_sfr[0x0CU] |= 0x10U;

    /* PIE1 / PIE2, Bank 1 mirrors of PIR1 / PIR2, reset to 0. */
    pic16f7x_sim_sfr[0x8CU] = PIC_PIE1_POR_VALUE;
    pic16f7x_sim_sfr[0x8DU] = PIC_PIE2_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_T1CON]    = PIC_T1CON_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_T2CON]    = PIC_T2CON_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_ADCON0]   = PIC_ADCON0_POR_VALUE;
    pic16f7x_sim_sfr[PIC_REG_ADCON1]   = PIC_ADCON1_POR_VALUE;

    /* PORTA on POR reads as analog input '0' (DS30325 §4.1). */
    pic16f7x_sim_sfr[PIC_REG_PORTA]    = 0x00U;
    pic16f7x_sim_sfr[PIC_REG_PORTE]    = 0x00U;

    /* TRIS defaults: 1 = input, so PORTA:6 and others = 1. */
    pic16f7x_sim_sfr[PIC_REG_TRISA]    = 0x3FU;     /* 6-bit wide. */
    pic16f7x_sim_sfr[PIC_REG_TRISB]    = 0xFFU;
    pic16f7x_sim_sfr[PIC_REG_TRISC]    = 0xFFU;
    pic16f7x_sim_sfr[PIC_REG_TRISD]    = 0xFFU;
    pic16f7x_sim_sfr[PIC_REG_TRISE]    = 0x07U;
...
static void sim_step_timer0(void)
{
    ...
    uint8_t option = pic16f7x_sim_sfr[PIC_REG_OPTION];
    ...
    /* OPTION_REG<PS2:PS0> prescaler mapping (DS30325 §5.0, Table 5-1). */
    ...
    pic16f7x_sim_sfr[PIC_REG_TMR0] = t0;
}
...
static void sim_step_timer1(void)
{
    /* T1CON layout (DS30325 Register 6-1):
     *   bit 0  TMR1ON
     *   bit 1  TMR1CS
     *   bit 2  T1SYNC
     *   bit 3  T1OSCEN
     ...
     *   bit 5  T1CKPS1
     */
    uint8_t t1con = pic16f7x_sim_sfr[PIC_REG_T1CON];
    ...
    uint8_t lo = pic16f7x_sim_sfr[PIC_REG_TMR1L];
    uint8_t hi = pic16f7x_sim_sfr[PIC_REG_TMR1H];
...
static void sim_step_timer2(void)
{
    /* T2CON layout (DS30325 Register 7-1):
     *   bit 0  T2CKPS0
     *   bit 1  T2CKPS1
     *   bit 2  TMR2ON
     *   bit 3..6 TOUTPS0..TOUTPS3
     */
    uint8_t t2con = pic16f7x_sim_sfr[PIC_REG_T2CON];
    if (!(t2con & 0x04U)) return;     /* TMR2ON = 0 → stopped. */

    /* T2CKPS1:T2CKPS0 → 1:1, 1:4, 1:16, 1:16 (DS30325 Register 7-1). */
    ...
    /* Read PR2 (Bank 1, address 0x92 per DS30325 §7.0). */
    uint8_t pr2 = pic16f7x_sim_sfr[0x92U];
    ...
    /* Increment TMR2. DS30325 §7.0: TMR2 increments until it matches
    ...
    pic16f7x_sim_sfr[PIC_REG_TMR2] = t2;
}
 ...
 */
void pic16f7x_sim_drive_input(char port, uint8_t pin, uint8_t level)
{
    if (pin > 7U) return;
    ...
    uint8_t portval = pic16f7x_sim_sfr[pa];
    if (level) portval |= mask;
    else portval &= (uint8_t)~mask;
    pic16f7x_sim_sfr[pa] = portval;
}
 ...
 */
uint8_t pic16f7x_sim_read_output(char port, uint8_t pin)
{
    if (pin > 7U) return 0U;
    uint8_t idx  = port_index(port);
    uint8_t mask = (uint8_t)(1U << pin);
 ...
 *        unregister.
 */
void pic16f7x_sim_set_irq_callback(pic16f7x_sim_irq_cb_t cb)
{
    sim_irq_cb = cb;
}
 ...
 */
void pic16f7x_sim_drive_usart_rx(uint8_t data)
{
    /* Place the byte in RCREG (0x1A, DS30325 §10.x). */
    pic16f7x_sim_sfr[PIC_REG_RCREG] = data;
    /* Set PIR1<RCIF> (bit 5). */
    pic16f7x_sim_sfr[0x0CU] |= 0x20U;
    if (sim_irq_cb) sim_irq_cb();
}
...
{
    /* Place byte in SSPBUF (0x13, DS30325 §9.x). */
    pic16f7x_sim_sfr[PIC_REG_SSPBUF] = data;
    /* Set SSPSTAT<BF> (Bank 1, addr 0x94, bit 0). */
    {
        uint8_t prev = (pic16f7x_sim_sfr[PIC_REG_STATUS] >> 5) & 0x03U;
        pic16f7x_sim_sfr[PIC_REG_STATUS] =
            (uint8_t)((pic16f7x_sim_sfr[PIC_REG_STATUS] & 0x1FU) | (1U << 5));
        pic16f7x_sim_sfr[0x94U] |= 0x01U;
        pic16f7x_sim_sfr[PIC_REG_STATUS] =
            (uint8_t)((pic16f7x_sim_sfr[PIC_REG_STATUS] & 0x1FU) | (prev << 5));
    }
    /* Set PIR1<SSPIF> (bit 3). */
    pic16f7x_sim_sfr[0x0CU] |= 0x08U;
    if (sim_irq_cb) sim_irq_cb();
}
