/*
 * Dedicated IRQ-core smoke test (docs/adding-a-device.md §5.5): enable
 * one Timer0 interrupt, confirm it fires with INTCON<TMR0IE>/T0CON<TMR0ON>
 * reading back set (DS39609B §9.0/§11.0). The ISR toggles RB0, counts
 * overflows, and drives the RA0 PASS marker from the second overflow:
 * under MPLAB SIM the main loop can starve behind firing ISRs and never
 * reach report().
 */
 ...
 * INTCON = 0xE0 (GIE/GIEL/TMR0IE); INTCON2 = 0xFF (POR) with TMR0IP
 * set (priority high); RCON = 0x9C (POR 0x1C | IPEN); TRISB = 0xFE
 * (RB0 out); LATB = 0x00 (starts low).
 */
...
extern void pic18f6520_harness_halt(void);
...
int main(void)
{
    ...
    /* 1. RB0 as output, start low (writes go through LATB, DS39609B §10.0). */
     ...
     *    vector (INTCON2<TMR0IP>): the reset default is low priority,
     *    and the dedicated-vector high path is the one the mdb-proven
     *    TIMER2 gates exercise; a source on the low vector is more
     *    readily starved under MPLAB SIM's interrupt-servicing model.
     *    This is the documented EPIC_IRQ_* contract. */
