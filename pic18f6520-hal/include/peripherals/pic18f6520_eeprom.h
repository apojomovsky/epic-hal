/*
 * Data EEPROM driver (DS39609B §7.0), 1024 bytes. Unlike the 4550/2520
 * families (256 B, EEADR only), this part's 1 KB array is addressed by
 * the EEADRH:EEADR pair, so every address is 16-bit. PIC18 moves the
...
#ifndef PIC18F6520_EEPROM_H
#define PIC18F6520_EEPROM_H
 ...
 *         `addr`, sets EECON1<RD> (with EEPGD=0), and returns the byte
 *         from EEDATA. Per §7.1: the address must be loaded first, then
 *         RD.
 * @param addr the data EEPROM address, 0..1023 (EEADRH:EEADR, 10-bit).
 ...
 * @param start the first data EEPROM address, 0..1023.
