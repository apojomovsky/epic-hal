/*
 * MSSP driver: SPI master/slave + I2C master/slave (DS39609B §17.0).
...
#ifndef PIC18F6520_SSP_H
#define PIC18F6520_SSP_H

#include "pic18f6520_hal.h"
#include "pic18f6520_sfr.h"

/**
 * @brief SSP mode select (SSPCON1<3:0>, DS39609B Registers 17-2/17-4).
 */
typedef enum {
...
/**
 * @brief SPI clock-edge select (SSPSTAT<CKE>, DS39609B §17.2.1).
 */
typedef enum {
...
/**
 * @brief SPI clock polarity (SSPCON1<CKP>, DS39609B §17.2.4).
 */
typedef enum {
...
/**
 * @brief SPI input-data sample phase (SSPSTAT<SMP>, DS39609B §17.2.4).
 *        Master mode only.
 */
typedef enum {
...
/**
 * @brief Clear the WCOL flag (must be done in software per §17.2.2).
 */
void     EPIC_SSP_ClearWriteCollision(void);

/**
 * @brief  Compute SSPADD for an I²C master baud rate.
 *         DS39609B §17.4.2: Fscl = Fosc / (4 x (SSPADD + 1))
