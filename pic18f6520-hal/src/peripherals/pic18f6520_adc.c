/*
 * A/D converter driver, implementation (DS39609B §19.0). All ADC
 * registers are in the Access Bank (0xFC0-0xFC4), no bank switching.
 * Conversion completion (GO/DONE clear + ADIF) is proven via mdb; the
 * host sim verifies register programming only.
 */
...
EPIC_StatusTypeDef EPIC_ADC_Init(const ADC_HandleTypeDef *h)
{
    ...
    /* ADCON2 (Register 19-3): ADFM | ADCS2:ADCS0 only. This part has
     * no ACQT field (bits 3:6 unimplemented, DFP/EDC confirmed), so the
     * Acquisition handle field is ignored (internally timed sample-
     * and-hold, DS39609B §19.0). */
    uint8_t adcon2 = 0U;
 ...
 * @param ch Channel number (0..11, 4-bit CHS field).
 */
...
uint16_t EPIC_ADC_Read(void)
{
     ...
     * 0..1023 result (DS39609B Register 19-3 + Figure 19-4). */
