/*
 * Comparator driver, implementation (DS39609B §20.0). CMCON is in the
 * Access Bank (0xFB4), no bank switching. Output changes (C1OUT/C2OUT +
 * CMIF) are proven via mdb; the host sim verifies register programming
 * only.
 */
...
EPIC_StatusTypeDef EPIC_COMP_Init(const COMP_HandleTypeDef *h)
{
    ...
    /* Build CMCON (Register 20-1, Access Bank 0xFB4).
     *   bits 2:0 CM2:CM0 (mode)
     *   bit 3   CIS
     *   bit 4   C1INV
     *   bit 5   C2INV
     *   bits 7:6 C2OUT:C1OUT (read-only status, hardware-set). */
 ...
 *         flag, restore CMCON to its power-on value (0x00, Reset mode)
 *         and drop the stored handle.
 * @return EPIC_OK.
 */
EPIC_StatusTypeDef EPIC_COMP_DeInit(void)
{
    ...
    EPIC_IRQ_ClearFlag(PIC18_IRQ_CMP);
    epic_sfr_write8(PIC_REG_CMCON, PIC_CMCON_POR_VALUE);   /* 0x00, Reset mode. */
    g_comp = NULL;
    return EPIC_OK;
}
