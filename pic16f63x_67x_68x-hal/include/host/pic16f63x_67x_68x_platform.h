 ...
 * target/pic16f63x_67x_68x_platform.h); the include path picks which
 * resolves, so pic16f63x_67x_68x_hal.h includes this name
 * unconditionally with no #ifdef. SFR access indexes the 512-byte
 * memory-backed pic16f63x_67x_68x_sim_sfr[]
 * (src/sim/pic16f63x_67x_68x_sim.c), so tests can poke registers
 * directly. */
 ...
 * src/sim/pic16f63x_67x_68x_sim.c. Banked addresses index straight in
 * (0x119 reads element 0x119); no banking is modelled at the array
 * level. */
extern uint8_t pic16f63x_67x_68x_sim_sfr[0x200];
...
#define EPIC_SFR_PTR(addr)       (&pic16f63x_67x_68x_sim_sfr[(uint16_t)(addr)])
#define epic_sfr_read8(addr)     (pic16f63x_67x_68x_sim_sfr[(uint16_t)(addr)])
#define epic_sfr_write8(addr, v) \
    do { pic16f63x_67x_68x_sim_sfr[(uint16_t)(addr)] = (uint8_t)(v); } while (0)

/* Address of a register as a uint8_t lvalue (read/write/RMW). */
#define EPIC_REG8(addr)          (pic16f63x_67x_68x_sim_sfr[(uint16_t)(addr)])

/* PIE1 (0x8C) / PIE2 (0x8D) enable/disable, direct read-modify-write:
 * the simulated register file is a plain array, so none of
 * target/pic16f63x_67x_68x_platform.h's inline-asm banking path
 * applies here. */
#define EPIC_PIE_ENABLE_BIT(is_pir2, mask) \
    do { \
        if (is_pir2) { pic16f63x_67x_68x_sim_sfr[0x8DU] |= (uint8_t)(mask); } \
        else         { pic16f63x_67x_68x_sim_sfr[0x8CU] |= (uint8_t)(mask); } \
    } while (0)
...
#define EPIC_PIE1_READ_TMR1IE(out_var) ((out_var) = pic16f63x_67x_68x_sim_sfr[0x8CU])
