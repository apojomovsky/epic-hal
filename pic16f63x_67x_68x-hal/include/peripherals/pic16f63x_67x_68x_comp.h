...
typedef enum {
    ...
    COMP_INPUT_REF     = 0x1U,   /**< Internal reference (see RefSource). */
} COMP_InputSourceTypeDef;

/**
 * @brief Comparator internal reference source (VRCON<CxVREN>).
 */
typedef enum {
    COMP_REF_ABSOLUTE   = 0x0U,   /**< 0.6 V fixed reference. */
    COMP_REF_CVREF      = 0x1U,   /**< CVREF from VRCON. */
...
typedef struct {
    ...
    COMP_RefSourceTypeDef    RefSource;     /**< VRCON<CxVREN>. */
    bool                     Inverted;      /**< CxPOL. */
 ...
 *        (CM2CON1<C2SYNC>). Recommended when the comparator gates
 *        Timer1, to avoid missed increments.
...
#endif /* PIC16F63X_67X_68X_COMP_H */
