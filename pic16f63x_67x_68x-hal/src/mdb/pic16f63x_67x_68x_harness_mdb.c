...
/**
 * @brief Harness start-up (sim target): stores the cycle bound, arms
 *        RA0 as the marker output (driven low = not-yet-passed), and
 *        clears any stale EEPROM completion flag from a previous run.
...
{
    /* Magic-string dispatch, trimmed for flash: comparing the full
     * marker costs ~200 words on a 1K part. The only callers are
     * epic_harness_report's two fixed literals, so the distinctive
     * bytes pin the verdict: "...RESULT: PASS\n\0" vs "...RESULT:
     * FAIL\n\0" differ at fmt[21] ('P'/'F') and fmt[24] ('S'/'L'); the
     * shared "EPIC_HA" prefix and the ": " separator anchor the marker
     * shape. */
 ...
 */
void pic16f63x_67x_68x_harness_halt(void)
{
    for (;;) {
        /* nothing */
    }
