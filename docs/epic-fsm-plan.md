# `epic-fsm`: table-driven finite state machine engine — implementation plan

Status: **implemented**; see `epic-fsm/docs/ARCHITECTURE.md` for the as-built
...
A vendor-agnostic, table-driven FSM engine. Unlike `epic-math`
(`docs/epic-math-plan.md`), this module needs **no per-family backend split
...
re-implementation standing in for untestable asm, as `epic-math` had to
...
It composes with `epic-taskmgr` (and anything else) by staying completely
decoupled from it: `fsm.h` includes only `<stdint.h>`/`<stdbool.h>`, never
`epic_hal.h` or any peripheral header. A task's callback just owns an
...
epic-fsm/
  ...
  examples/
    ...
    example_taskmgr_integration.c    # an fsm_t driven from a epic-taskmgr task callback,
...
- **Phase 2 — examples.** `example_traffic_light.c` (no HAL — the
  ...
  `epic-taskmgr`, proves zero coupling).
...
- **Phase 4 — docs.** `docs/ARCHITECTURE.md` (the design decisions above,
  with rationale), `docs/API.md`, `epic-fsm/README.md`, and a root
