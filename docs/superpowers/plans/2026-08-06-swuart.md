...
/* N from docs/superpowers/plans/probe-swuart-isr-budget.md's verdict. */
...
static uint8_t g_oversample_n = 4u; /* Task 1's probe verdict. */
...
The tick rate is `baud * N`, where `N` is the oversample factor
`docs/superpowers/plans/probe-swuart-isr-budget.md` decided (4, unless
that probe found the ISR too large on PIC16F87XA at 20 MHz, in which
case 3). At 9600 baud and N=4: a tick every 26.04 microseconds.
...
- Real-target `.hex` builds clean for `16F877A`, `18F4550`, and `16F1937`
  via `epic_build.py`.
- The probe doc records a real verdict (YES at N=4, or NO with N=3
  adopted), not an assumption.
