...
/* epic_harness_tick() advances exactly one INSTRUCTION CYCLE per call
 * (see core/epic_harness.h), not one software UART tick. One Timer1
 * overflow takes CYCLES_PER_TICK real instruction cycles, the same
 * value epic_swuart.c's compute_reload() computes internally as
 * ticks_per_period. Advancing OVERSAMPLE_N raw epic_harness_tick()
 * calls per "tick" (174 cycles at 20 MHz/9600 baud/N=3) leaves the
 * interrupt never firing at all within any test's short run; the
 * symptom is every sampled pin reading the Init-time idle-high default
 * regardless of what the state machine should be driving. */
