# `epic-usb`: USB CDC-ACM device stack for the PIC18F2455/2550/4455/4550 family — implementation plan

Status: **Phase 1/2 done** (M-Stack vendored, wrapper + host-stub + unit
tests, see "Phase 2 findings" below and `epic-usb/docs/ARCHITECTURE.md` for
the as-built version), **Phase 3 (real-silicon bring-up,
`mcu/pic18fxx5x-usb-mplabx/`) not started**, matching `epic-usb/README.md`'s
...
## What this is, and why it breaks the pattern every other `epic-*` module set

Every other module in this repo (`epic-fsm`, `epic-debounce`, `epic-adcfilter`,
`epic-math`) is either genuinely vendor-agnostic C or, at worst, a
per-family backend split with a portable-C reference. `epic-usb` is neither:
...
**Vendoring — done.** M-Stack is vendored into `epic-usb/third_party/m-stack/`
...
Do not copy M-Stack files into `epic-usb/src/` — `epic_usb.c` calls into
...
- **No built-in ring buffer or `read()`/`write()`.** The real API is raw
  ...
  plan's design: `epic_usb.c` genuinely has to own the ring buffering on
  ...
  anything better.
- **`epic_usb_connected()` (DTR) must be backed by
  ...
  `wValue & 0x1` = DTR state). `epic_usb.c` should track this in a static
...
## Public API — mirrors `epic-serial` deliberately

Anyone who already knows `epic_serial_*` should be able to pick this up
...
```c
/* epic-usb/include/epic_usb.h */
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

epic_status_t epic_usb_init(void);

/* Must be called frequently (every main-loop iteration, or from a
 * epic-taskmgr task with a short period — see "Servicing cadence" below).
 ...
 * epic_usb_connected() is true. */
void epic_usb_service(void);

/* Same contract as epic_serial_write/read/available/flush: */
 ...
 * anyone listening" need this, not just epic_usb_init() having returned. */
...
on here on the chance it's needed. Same reasoning `epic-debounce`'s plan
...
`epic_usb_service()`, both valid, caller's choice:
...
- **`epic-taskmgr` task**: `task_spawn()` it with a short period (a couple
  of ticks). Fits naturally if the firmware is already structured around
  the cooperative scheduler, the same way `epic-fsm`'s
...
`epic-serial`'s host build works because a UART loopback is a faithful
...
- The ring-buffer and connection-state bookkeeping inside `epic_usb.c`
  ...
  same pattern as `epic-tick`'s simulated Timer2: exercises the logic this
...
epic-usb/
  ...
  include/
    epic_usb.h
  src/
    epic_usb.c                       # ring buffers + epic_usb_* wrapper, real usb_hal target only
    epic_usb_host_stub.c             # host build: fakes connected()/service() enough to unit-test buffers
  third_party/
    m-stack/                         # vendored, Apache-2.0 arm of its dual license, untouched
  examples/
    example_usb_echo.c               # epic_usb_read() -> epic_usb_write() loopback, the smoke test
  tests/
    test_epic_usb.c                  # ring buffer fill/drain, connected() state transitions, against the stub
  mcu/
...
- **Phase 2 — wrapper + host-stub + unit tests. DONE**, with a bonus:
  ...
  findings" below for what that turned up. `epic_usb.h`/`.c`, the
  host-stub backend, `tests/test_epic_usb.c` (25 checks, all passing via
...
- **Phase 4 — PID + docs.** File the pid.codes PR for a real PID, swap it
  ...
  `docs/API.md`, `epic-usb/README.md`, a root `README.md` component-table
  row.
...
- `epic_usb_init()` returns `void`, not `epic_status_t` as first drafted.
  ...
  `epic_fsm_init()`'s precedent of not reaching for `epic-common`'s status
  enum when nothing can actually fail.
- M-Stack's required `app_*`-named callbacks are implemented as
  `epic_usb_*_cb` in `epic_usb.c` (mapped via macros in this module's own
  ...
  convention directly.
- The host-stub test double (`epic_usb_host_stub.c`) is a genuinely
  separate implementation from `epic_usb.c`, as this plan's "Host build
  story" already called for — not a shortcut discovered during
  implementation, confirmed as the right call once `epic_usb.c` turned out
